...
import com.example.sasha.finalsoftware.ui.NameQuizActivity;
...
public class CensusSearcher {
    ...
    private NameQuizActivity quizActivity;
    ...
    public CensusSearcher(Context context, NameQuizActivity com) {
        this.context = context;
        nameList = new ArrayList<Name>();
        queue = Volley.newRequestQueue(context);
        quizActivity = com;
    ...
    public void getPopularName(int year, String gender) {
        String url = "http://ukko.d.umn.edu:9091/getPopularName/"
                + year + "/" + wildcardToRegex(gender);
        JsonArrayRequest req = new JsonArrayRequest
                (Request.Method.GET, url, null, new Response.Listener<JSONArray>() {
                    @Override
                    public void onResponse(JSONArray response) {
                        Log.d("git_sum", "response in getPopularName");
                        newname = fillPopularName(response);
                        Log.d("git_sum", "name about to be sent to quizactivity: " + newname);
                        quizActivity.updateGUI();
                    }
                }, new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        Log.d("git_sum", "error in getPopularName" + error);
                    }
                });
    ...
    public void searchForName(String name, Period period) {
        ...
        // behavior we want
        String url = "http://ukko.d.umn.edu:9091/getName/"
    ...
    private void fillNameList(JSONArray jsonArray) {
        ...
        Name name;
        Log.d("git_sum", "fillname Array size:" + jsonArray.length());
    ...
    private String fillPopularName(JSONArray jsonArray) {
        JSONObject jsonObject;
        String tempname = "temp";
        Name name;
        Log.d("git_sum", "array length:" + jsonArray.length());
        try {
            for (int i = 0, len = jsonArray.length(); i < len; i++) {
                jsonObject = jsonArray.getJSONObject(i);
                tempname = ((String) jsonObject.get("name"));
                Log.d("git_sum", "name received:" + tempname);
                Log.d("name", tempname);
            }
        }
        catch(JSONException e) {
            Log.d("git_sum", "exception in fill popular name " + e);
        }
        return tempname;
    }

    public String getName(){
        Log.d("git_sum", "name about to be passed to updateUI: " + newname);
        return newname;
    }
}
